...
| `config/sponsors.ts` | 首页合作伙伴的名称、Logo、可选链接及显示顺序 |
...
首页合作伙伴由 `config/sponsors.ts` 中的 `sponsors` 数组控制。每项填写 `name` 和 `logo`，可选填 `url` 与数值 `order`（越小越靠前）；图片放在 `public/sponsors/`，用 `/sponsors/文件名.svg` 引用。有 `url` 时卡片可点击；数组为空时整个区域隐藏。仓库中的三个 `DEMO` 项及 Logo 仅供预览，上线前请替换。
